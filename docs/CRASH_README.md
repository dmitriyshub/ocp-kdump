...
- If our kernel is in the same version of the node kernel, we can enable the relevant yum repos and install the kernel-debuginfo and kernel-debuginfo-common
...
- If our kernel is different from the nodes, we can download rpm packages from the [Customer Portal](https://access.redhat.com/downloads/content/package-browser)
...
- Check if you new vmlinux created under `/usr/lib/debug/lib/modules/<kernel-version>/vmlinux`
